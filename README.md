...
Your mission is to program C3PO to compute the odds that the Millennium Falcon reaches Endor in time and saves the galaxy.
...
The first JSON file contains the navigation data of the Millennium Falcon
```json
{
  "autonomy": 6, 
  "routes": [
...
- `x (0 < x < 1)` if the Millennium Falcon can reach Endor before the Death Star annihilates Endor but might be captured by bounty hunters.
...
### Example 1
**[millennium-falcon.json](examples/example1/millennium-falcon.json)**
```
{
  "autonomy": 6, 
  "routes": [
...
`giveMeTheOdds(empireJsonFile)` should return 0 as The Millennium Falcon cannot go from Tatooine to Endor in 7 days or less (the Millennium Falcon must refuel for 1 day on either Dagobah or Hoth).

### Example 2
**[millennium-falcon.json](examples/example2/millennium-falcon.json)**: same as above
...
`giveMeTheOdds(empireJsonFile)` should return 0.81 as The Millennium Falcon can go from Tatooine to Endor in 8 days with the following plan:
...
### Example 3
**[millennium-falcon.json](examples/example3/millennium-falcon.json)**: same as above
...
`giveMeTheOdds(empireJsonFile)` should return 0.9 as The Millennium Falcon can go from Tatooine to Endor in 8 days with the following plan:
...
### Example 4
**[millennium-falcon.json](examples/example4/millennium-falcon.json)**: same as above
...
`giveMeTheOdds(empireJsonFile)` should return 1 as The Millennium Falcon can go from Tatooine to Endor in 10 days and avoid Bounty Hunters with the following plans:
